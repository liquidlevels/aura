...
const FrequencyScreen = () => {
  ...
  const filterDataFromApi = (
    data: any[],
    filterCriteria: { heartRate: any; date: any }
  ) => {
    return data
      .filter((item) => {
        // Filtrar por frecuencia cardíaca, si se especifica
        if (
          filterCriteria.heartRate &&
          item.heart_rate !== filterCriteria.heartRate
        ) {
          return false;
        }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={[styles.date, { textAlign: "left", paddingLeft: 10 }]}>
        {date}
      </Text>

      <Text
        style={[styles.normalFrequency, { textAlign: "left", paddingLeft: 10 }]}
      >
        {isNormalFrequency(averageFrequency)
          ? "Frecuencia Normal"
          : "Frecuencia Anormal"}
      </Text>
      ...
      <View style={{ padding: 20 }}>
        ...
        {/* Slider que aparece/oculta según el estado */}
        {isSliderVisible && (
        ...
        <View style={styles.rangeControls}>
          <TouchableOpacity
            ...
            onPress={() => setRange(Math.max(range - 1, 3))}
          >
            <Text style={styles.rangeButtonText}>Ver Menos</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.rangeButton}
            onPress={() => setRange(Math.min(range + 1, data.length))}
          >
      ...
      <Text style={styles.description}>
        Las frecuencias cardiacas normales varían entre 60 y 100 latidos por
        minuto (bpm) en reposo para la mayoría de los adultos. Valores fuera de
        este rango pueden indicar problemas cardíacos o un estado de salud fuera
        de lo normal.
      </Text>
